...
def gather_module_classes(package):
    """
    Traverse all submodules of a package and gather information
    about classes—including their fully qualified names, attributes,
    methods (with signatures), and inheritance.
    ...
    for importer, modname, ispkg in pkgutil.walk_packages(package.__path__, package.__name__ + "."):
        ...
        for name, obj in inspect.getmembers(mod, inspect.isclass):
            ...
            # Gather inheritance information (list of base classes as fully qualified names).
...
def save_classes_info_to_file(modules_info, filename):
    """
    Saves the gathered class information (including fully qualified names and
    inheritance details) grouped by module to a file.
    """
    with open(filename, "w", encoding="utf-8") as f:
        for modname in sorted(modules_info.keys()):
            ...
            for cls in modules_info[modname]:
                ...
                # Print the fully qualified class name.
                f.write(f"{kind}: {cls['qualified_name']}\n")
...
output_file = "sklearn_uml_output_fullClasspath.txt"
